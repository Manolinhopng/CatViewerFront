...
export async function getLikes(image_id) {
    const response = await fetch(`${API_URL}/api/like/${image_id}`);
    return response.json();
}
export async function toggleLike(image_id) {
...
export async function getBreedLikes(breed_id) {
    const response = await fetch(`${API_URL}/api/breedLikes/${breed_id}`);
    return response.json();
}
export async function toggleBreedLike(breed_id) {
    // user_id ya no se envía: el backend lo extrae del JWT
